    ...
    for duration in time_intervals:
        for variant in ["light_bg", "dark_bg"]:
            ...
            for frame_count in range(total_frames):
                ...
                # Function to draw a semi-transparent ring
                def draw_semi_transparent_ring(image, center, radius, start_angle, end_angle, color, thickness, opacity):
                    overlay = image.copy()
                ...
                if hour_angle > 0:
                    draw_semi_transparent_ring(frame, center, outer_radius, start_angle, hour_angle, red_color, ring_thickness, opacity)

                # Draw the minute ring (Green) only if minutes is non-zero
                if minute_angle > 0:
                    draw_semi_transparent_ring(frame, center, outer_radius - ring_thickness * 2, start_angle, minute_angle, green_color, ring_thickness, opacity)

                # Draw the second ring (Blue) only if seconds is non-zero
                if second_angle > 0:
                    draw_semi_transparent_ring(frame, center, outer_radius - ring_thickness * 4, start_angle, second_angle, blue_color, ring_thickness, opacity)
